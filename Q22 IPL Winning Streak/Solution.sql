...
cte_final AS (
	SELECT 
		*,
		COUNT(*) OVER(PARTITION BY teams, diff  ORDER BY teams, dates) AS streak
	FROM cte_diff
)
SELECT 
	t.teams, COALESCE(MAX(streak), 0) AS winning_streak
FROM cte_final f
RIGHT JOIN cte_teams t ON t.teams = f.teams
GROUP BY teams
