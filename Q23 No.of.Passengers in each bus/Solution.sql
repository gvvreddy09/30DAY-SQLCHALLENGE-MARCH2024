...
/*CREATE TEMPORARY TABLE cte_data (
	SELECT *,
		(SELECT COUNT(passenger_id) FROM passengers WHERE arrival_time <= b.arrival_time) AS total_passenger
	FROM buses b
	ORDER BY bus_id
);*/
WITH RECURSIVE cte AS (
	WITH cte_data AS(
		SELECT ROW_NUMBER() OVER(ORDER BY arrival_time) AS rn, b.*,
			(SELECT COUNT(passenger_id) FROM passengers WHERE arrival_time <= b.arrival_time) AS total_passenger
		FROM buses b
		ORDER BY bus_id)
    
	SELECT rn, bus_id,
		LEAST(capacity, total_passenger) AS onboarded_bus,
		LEAST(capacity, total_passenger) AS total_onboarded_bus
    FROM cte_data WHERE bus_id = 1
	UNION
    SELECT d.rn, d.bus_id,
		LEAST(d.capacity, (d.total_passenger-c.total_onboarded_bus)) AS onboarded_bus,
		c.total_onboarded_bus + LEAST(d.capacity, (d.total_passenger-c.total_onboarded_bus)) AS total_onboarded_bus
    FROM cte c JOIN cte_data d ON c.rn + 1 = d.rn
)
SELECT bus_id, onboarded_bus AS passenger_cnt FROM cte ORDER BY bus_id;
